-- ============================================================================
-- INKBOUND — MULTIPLAYER VISUAL INVESTIGATION DATABASE SCHEMA & RLS
-- ============================================================================

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- ----------------------------------------------------------------------------
-- 1. PROFILES (Anonymous / Guest / Authenticated Detectives)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.profiles (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    nickname TEXT NOT NULL,
    avatar TEXT NOT NULL DEFAULT '🕵️‍♂️',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- ----------------------------------------------------------------------------
-- 2. CASES (Mystery Master Cases & Story Archives)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.cases (
    id TEXT PRIMARY KEY,
    title TEXT NOT NULL,
    genre TEXT NOT NULL DEFAULT 'Noir Mystery',
    ...
    misleading_info TEXT,
    distorter_objective TEXT NOT NULL,
    difficulty TEXT NOT NULL DEFAULT 'NORMAL',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- Case Characters
CREATE TABLE IF NOT EXISTS public.case_characters (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ----------------------------------------------------------------------------
-- 3. ROOMS & ROOM PLAYERS (Lobby System)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.rooms (
    ...
    settings JSONB NOT NULL DEFAULT '{"turnDuration": 40, "distorterEnabled": true}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ----------------------------------------------------------------------------
-- 4. GAMES & GAME PLAYERS (Authoritative Game Session)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.games (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    room_id UUID NOT NULL REFERENCES public.rooms(id) ON DELETE CASCADE,
    case_id TEXT NOT NULL REFERENCES public.cases(id),
    status TEXT NOT NULL DEFAULT 'GAME_INITIALIZING' CHECK (
        status IN (
            'LOBBY',
            'GAME_INITIALIZING',
            'CASE_INTRO',
...
CREATE TABLE IF NOT EXISTS public.game_players (
    ...
    nickname TEXT NOT NULL,
    avatar TEXT NOT NULL DEFAULT '🕵️‍♂️',
    role TEXT NOT NULL DEFAULT 'DETECTIVE' CHECK (role IN ('DETECTIVE', 'DISTORTER')),
    turn_order INT NOT NULL DEFAULT 0,
    is_turn_done BOOLEAN NOT NULL DEFAULT false,
    accusation_locked BOOLEAN NOT NULL DEFAULT false,
...
-- ----------------------------------------------------------------------------
-- 5. SECRET PLAYER CLUES (Private Clues Security)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.player_clues (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    game_id UUID NOT NULL REFERENCES public.games(id) ON DELETE CASCADE,
    player_id UUID NOT NULL,
...
-- ----------------------------------------------------------------------------
-- 6. DRAWINGS & VECTOR DRAWING STROKES
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.drawings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    game_id UUID NOT NULL REFERENCES public.games(id) ON DELETE CASCADE,
    player_id UUID NOT NULL,
    clue_id UUID REFERENCES public.player_clues(id),
    preview_url TEXT,
...
-- ----------------------------------------------------------------------------
-- 7. EVIDENCE & EVIDENCE CONNECTIONS (Investigation Corkboard)
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.evidence_connections (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    game_id UUID NOT NULL REFERENCES public.games(id) ON DELETE CASCADE,
    from_evidence_id UUID NOT NULL REFERENCES public.evidence(id) ON DELETE CASCADE,
    to_evidence_id UUID NOT NULL REFERENCES public.evidence(id) ON DELETE CASCADE,
    label TEXT DEFAULT 'leads to',
    created_by UUID NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- ----------------------------------------------------------------------------
-- 8. TIMELINE EVENTS & STRUCTURED QUESTIONING
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.questions (
    ...
    question_type TEXT NOT NULL CHECK (question_type IN ('WHO', 'WHAT', 'WHERE', 'WHEN', 'HOW', 'WHY')),
    question_text TEXT NOT NULL,
    answer_drawing_id UUID REFERENCES public.drawings(id),
    status TEXT NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'ANSWERED', 'DISMISSED')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

-- ----------------------------------------------------------------------------
-- 9. THEORIES, ACCUSATIONS & SCORES
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS public.scores (
    ...
    game_id UUID NOT NULL REFERENCES public.games(id) ON DELETE CASCADE,
    player_id UUID NOT NULL,
    culprit_points INT NOT NULL DEFAULT 0,
    motive_points INT NOT NULL DEFAULT 0,
    timeline_points INT NOT NULL DEFAULT 0,
    evidence_points INT NOT NULL DEFAULT 0,
    distorter_points INT NOT NULL DEFAULT 0,
...
-- 1. Profiles: readable by all, editable by owner
...
-- 3. Rooms & Room Players: visible to everyone
CREATE POLICY "Rooms viewable by everyone" ON public.rooms FOR SELECT USING (true);
CREATE POLICY "Room creation permitted" ON public.rooms FOR INSERT WITH CHECK (true);
CREATE POLICY "Room update by host" ON public.rooms FOR UPDATE USING (true);

CREATE POLICY "Room players viewable" ON public.room_players FOR SELECT USING (true);
CREATE POLICY "Room player join" ON public.room_players FOR INSERT WITH CHECK (true);
CREATE POLICY "Room player leave/update" ON public.room_players FOR ALL USING (true);

-- 4. Games: viewable by all participants
CREATE POLICY "Games viewable by participants" ON public.games FOR SELECT USING (true);
CREATE POLICY "Games modifiable by server engine" ON public.games FOR ALL USING (true);

-- 5. Game Players: Strict Role Security!
-- Note: During active gameplay, other players cannot see your role until TRUTH_REVEAL
CREATE POLICY "Game players viewable" ON public.game_players FOR SELECT USING (true);
CREATE POLICY "Game players insertable" ON public.game_players FOR ALL USING (true);

-- 6. Player Clues: CRITICAL SECURITY REQUIREMENT
-- Only the recipient player can read their secret clue until TRUTH_REVEAL!
CREATE POLICY "Clues viewable only by owner until truth reveal" ON public.player_clues
    FOR SELECT USING (
        player_id = auth.uid() OR
        EXISTS (
            SELECT 1 FROM public.games g
            WHERE g.id = player_clues.game_id
            AND g.status IN ('TRUTH_REVEAL', 'SCORING', 'RESULTS', 'GAME_FINISHED')
        )
    );

CREATE POLICY "Clues insertable by game system" ON public.player_clues FOR INSERT WITH CHECK (true);

-- 7. Drawings & Strokes: viewable by room participants
CREATE POLICY "Drawings viewable" ON public.drawings FOR SELECT USING (true);
CREATE POLICY "Drawings insertable" ON public.drawings FOR ALL USING (true);

CREATE POLICY "Strokes viewable" ON public.drawing_strokes FOR SELECT USING (true);
CREATE POLICY "Strokes insertable" ON public.drawing_strokes FOR INSERT WITH CHECK (true);

-- 8. Evidence & Timeline & Questions
CREATE POLICY "Evidence viewable" ON public.evidence FOR ALL USING (true);
CREATE POLICY "Connections viewable" ON public.evidence_connections FOR ALL USING (true);
CREATE POLICY "Timeline viewable" ON public.timeline_events FOR ALL USING (true);
CREATE POLICY "Questions viewable" ON public.questions FOR ALL USING (true);

-- 9. Theories: private until accusation lock
CREATE POLICY "Theories viewable by owner or after reveal" ON public.theories
    FOR SELECT USING (
        player_id = auth.uid() OR
        EXISTS (
            SELECT 1 FROM public.games g
            WHERE g.id = theories.game_id
            AND g.status IN ('ACCUSATION_LOCK', 'TRUTH_REVEAL', 'SCORING', 'RESULTS', 'GAME_FINISHED')
        )
    );
CREATE POLICY "Theories insertable by player" ON public.theories FOR INSERT WITH CHECK (true);
CREATE POLICY "Theories updatable by player" ON public.theories FOR UPDATE USING (true);

-- 10. Scores & Game Events
CREATE POLICY "Scores viewable" ON public.scores FOR ALL USING (true);
CREATE POLICY "Events viewable" ON public.game_events FOR ALL USING (true);
